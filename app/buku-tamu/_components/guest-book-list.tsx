    ...
    >
      ...
      {filteredEntries.length === 0 ? (
        <motion.div variants={itemVariants} className="flex flex-col items-center justify-center py-16 text-center">
          ...
          {searchQuery && (
            <button onClick={() => { setSearchQuery(""); setCurrentPage(1); }} className="mt-4 text-sm font-semibold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700">
              Reset Pencarian
            </button>
          )}
        </motion.div>
        ...
        <>
          {/* DESKTOP VIEW: Table */}
          <motion.div variants={itemVariants} className="hidden overflow-x-auto rounded-2xl border border-slate-100 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 md:block transition-colors duration-300">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm text-slate-700 dark:text-slate-300">
                {currentEntries.map((entry) => (
                  <tr key={entry.id} className="transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/40">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-900 dark:text-slate-100">{entry.guestName}</div>
                      <div className="mt-0.5 flex items-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400">
                        ...
                        {entry.whatsapp && entry.whatsapp !== "-" ? (
                          <MaskedPhone phone={entry.whatsapp} />
                        ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900 dark:text-slate-100">{entry.intendedOfficer}</div>
                    </td>
                    <td className="px-6 py-4 max-w-xs truncate" title={entry.purpose}>
                      <div className="text-slate-600 dark:text-slate-400 line-clamp-2">{entry.purpose}</div>
                    </td>
                ...
                >
                  <div className="flex items-center justify-between p-4">
                    <div className="flex-1 min-w-0 pr-4">
                      <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm truncate">{entry.guestName}</h4>
                      <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                        ...
                        <span className="font-semibold text-slate-700 dark:text-slate-300 truncate">{entry.intendedOfficer}</span>
                      </div>
                    </div>
