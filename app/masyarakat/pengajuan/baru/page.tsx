...
import { getPublicServiceCatalog } from "@/lib/queries";
...
export default async function NewRequestPage() {
  ...
  return (
    <div className="space-y-5 md:space-y-6 animate-in fade-in duration-500">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-xl">
            ...
            <p className="text-xs sm:text-sm font-medium text-emerald-100/80 dark:text-slate-300 leading-relaxed">
              Pilih unit layanan dan lengkapi data permohonan Anda secara mudah, transparan, dan realtime.
            </p>
          </div>
          ...
          <div className="shrink-0">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs sm:text-sm px-4 py-2.5 backdrop-blur-md border border-white/20 transition-all hover:scale-[1.02] active:scale-95"
            >
      ...
      <NewRequestForm catalog={catalog} profile={profile} />
    </div>
  );
}
