    ...
    <div className="divide-y divide-slate-100 dark:divide-slate-800 dark:bg-slate-900 transition-colors duration-300">
      {requests.map((request: any) => (
        <div key={request.id} className="p-4 sm:p-5 flex flex-col gap-3.5">
          <div className="flex items-start justify-between gap-2.5">
            <div className="flex flex-col gap-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  onClick={() => handleCopyTicket(request.requestNumber, request.id)}
          ...
          <div className="flex flex-col gap-0.5 bg-slate-50/60 dark:bg-slate-800/40 p-2.5 rounded-xl border border-slate-100/80 dark:border-slate-800">
            <span className="text-[11px] sm:text-xs font-bold text-slate-800 dark:text-slate-200 line-clamp-1">
