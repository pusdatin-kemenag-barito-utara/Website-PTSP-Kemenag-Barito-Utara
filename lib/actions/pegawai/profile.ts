...
// Initialize S3 client for Supabase Storage (using S3 compatibility)
...
export async function updatePegawaiAvatar(formData: FormData) {
  try {
    ...
    // Since regular users might not have RLS permission to upload to 'avatars',
    // we use admin client or S3 client.
    // In AGENTS.md: Upload via createAdminClient() (service_role, bypasses RLS)
...
export async function updatePegawaiPassword(formData: FormData) {
  try {
    ...
    // The user's active session is used here.
