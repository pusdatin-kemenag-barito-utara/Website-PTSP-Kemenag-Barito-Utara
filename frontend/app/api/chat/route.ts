...
async function getDatabaseKnowledgeContext() {
  try {
    ...
    for (const s of activeServices) {
      dbText += `\n- Kategori/Layanan: ${s.name}`;
      if (s.requirements_text || s.requirementsText)
        dbText += ` (Persyaratan umum: ${s.requirements_text || s.requirementsText})`;

      const items = s.items || s.serviceItems || [];
      for (const item of items) {
        dbText += `\n  * Sub-Layanan: ${item.name}`;
        if (item.estimated_time || item.estimatedTime)
          dbText += ` | Estimasi: ${item.estimated_time || item.estimatedTime}`;
      }
    }
...
export async function POST(req: Request) {
  try {
    ...
    let fallbackText =
      "Halo Bapak/Ibu, salam dari PTSP Kemenag Barito Utara! Ada yang bisa kami bantu mengenai pendaftaran layanan, syarat dokumen, atau informasi keagamaan?";

    if (lastUserMsg.includes("nikah") || lastUserMsg.includes("kawin")) {
      fallbackText =
        "Untuk pendaftaran nikah dapat diakses melalui portal SIMKAH (simkah4.kemenag.go.id). Syarat umum meliputi formulir N1, N2, N4, FC KTP, KK, Akta Nikah/Cerai/Kematian (jika ada), serta foto 2x3 & 4x6 latar biru. Seluruh layanan PTSP gratis!";
    } else if (
      lastUserMsg.includes("halal") ||
      lastUserMsg.includes("sertifikat")
    ) {
      fallbackText =
        "Pendaftaran Sertifikasi Halal Gratis (SEHATI) dapat diajukan melalui portal ptsp.halal.go.id BPJPH. Persyaratan utama melampirkan NIB dan dokumen data usaha.";
    } else if (
      lastUserMsg.includes("ijazah") ||
      lastUserMsg.includes("legalisir")
    ) {
      fallbackText =
        "Untuk legalisir ijazah/STTB, silakan membawa Ijazah asli beserta fotokopi maksimal 5 lembar ke Kantor Kemenag Barito Utara pada jam kerja.";
    } else if (
      lastUserMsg.includes("pimpinan") ||
      lastUserMsg.includes("kepala") ||
      lastUserMsg.includes("pejabat")
    ) {
      fallbackText =
        "Kepala Kantor Kemenag Barito Utara saat ini dijabat oleh H. Arbaja, S.Ag., M.A.P, dan Kasubbag TU dijabat oleh Sony Anwari Husni, S.Pd.";
    } else if (
      lastUserMsg.includes("kontak") ||
      lastUserMsg.includes("alamat") ||
      lastUserMsg.includes("wa")
    ) {
      fallbackText =
        "Kantor Kemenag Barito Utara berlokasi di Jl. Ahmad Yani No. 126, Muara Teweh. WhatsApp Resmi SI-ATAK: 0851-1749-1212. Jam kerja: Senin-Kamis 07.30-16.00 WIB & Jumat 07.30-16.30 WIB.";
    }
    ...
    return NextResponse.json({
      content:
        "Halo Bapak/Ibu, selamat datang di Portal PTSP Kemenag Barito Utara! Silakan ajukan pertanyaan atau hubungi WhatsApp Call Center SI-ATAK di 0851-1749-1212 untuk informasi lebih lanjut.",
    });
  }
}
