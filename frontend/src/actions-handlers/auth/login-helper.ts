...
export async function getEmailByPhoneAction(phone: string) {
  ...
  try {
    ...
    // Fallback pseudo-email: pastikan format email persis sama dengan registrasi pemohon (p62... @ptsp.id)
    const phone62 = cleanPhone.startsWith("0") ? "62" + cleanPhone.slice(1) : cleanPhone;
    return { email: `p${phone62}@ptsp.id` };
  } catch (err) {
...
export async function handlePegawaiLoginAction(nip: string, password?: string, token?: string) {
  ...
  try {
    ...
    if (createError) {
      // Jika email sudah terdaftar di Supabase Auth, lanjutkan alur login
      if ((createError as any).code === "email_exists" || createError.status === 422 || createError.message?.includes("already been registered")) {
        return { email: pseudoEmail };
      }
