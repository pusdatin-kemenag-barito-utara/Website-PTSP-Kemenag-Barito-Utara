...
export function DocumentPreviewModal({
  ...
  useEffect(() => {
    ...
    // Fetch file as Blob via backend proxy (bypass CORS / private bucket)
    fetch(`${getClientApiBase()}/files/proxy?url=${encodeURIComponent(url)}`)
      .then(async (res) => {
        if (!res.ok) {
      ...
      .catch(async (err) => {
        try {
          ...
          if (!directRes.ok) throw new Error("Gagal mengunduh berkas untuk pratinjau");
