...
export function useWizardRequirements(startTransition: any) {
  ...
  const handleReorder = (itemId: number, newReqs: any[], setService: any) => {
    ...
    reorderTimeout.current[itemId] = setTimeout(() => {
      startTransition(async () => {
        const ids = updatedReqs.map((r: any) => r.id as number);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast.success(
          editingReq ? "Persyaratan Diperbarui" : "Persyaratan Ditambahkan",
        );
        if (editingReq) {
          setModalOpen(false);
          setEditingReq(null);
