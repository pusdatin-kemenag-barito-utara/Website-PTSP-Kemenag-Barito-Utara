...
export function ServiceWizardClient({
  ...
  const handleReorderItem = (itemId: any, direction: "up" | "down") => {
    ...
    [newItems[index], newItems[moveIndex]] = [
      newItems[moveIndex],
      newItems[index],
    ];
  ...
  const handleDeleteItem = (id: number) => {
    if (
      !confirm("Hapus item layanan ini beserta semua form dan persyaratannya?")
    )
      return;
  ...
  const handleReorderField = (
    ...
    const index = currentFields.findIndex((f: any) => f.id === fieldId);
    if (index === -1) return;
    ...
    const moveIndex = direction === "up" ? index - 1 : index + 1;
    [newFields[index], newFields[moveIndex]] = [
      newFields[moveIndex],
      newFields[index],
    ];

    const ids = newFields.map((f: any) => f.id.toString());
  ...
  const handleReorderReq = (
    reqId: any,
    direction: "up" | "down",
    currentReqs: any[],
  ) => {
    const sorted = [...currentReqs].sort(
      (a: any, b: any) => (a.sort_order ?? 0) - (b.sort_order ?? 0),
    );
  ...
  const handleExtensionChange = (ext: string, checked: boolean) => {
    setReqFormData((prev) => {
      const current = prev.allowed_extensions
        ? prev.allowed_extensions
            .split(",")
            .map((e: string) => e.trim())
            .filter(Boolean)
        : [];
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Kelola jenis-jenis layanan turunan di dalam layanan ini.
            </p>
          </div>
          <button
            onClick={() => {
            ...
            <div key={item.id} className="group">
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`p-2 rounded-lg transition-colors ${expandedItemId === item.id ? "bg-[#059669] text-white" : "bg-slate-100 text-slate-400 group-hover:bg-slate-200"}`}
                  >
                ...
                >
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-md text-[10px] font-bold mr-4 ${item.is_active ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                  >
          ...
          {items.length === 0 && (
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-slate-400 mt-1">
                Mulai dengan menambahkan item layanan pertama.
              </p>
            </div>
          )}
        </div>
